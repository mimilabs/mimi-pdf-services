...
def test_basic_template(access_token,
    ...
    headers = {"content-type": "application/json",
               "Authorization": ("Bearer " + access_token)}
    res = requests.post(f"{URL}/use_basic_template",
                        headers=headers,
                        data=json.dumps(data))
    if res.status_code == 200:
        filename = res.headers['content-disposition'].split('=')[1]
...
def test_prc_template(access_token, format="pdf"):
    ...
    headers = {"content-type": "application/json",
               "Authorization": ("Bearer " + access_token)}
    res = requests.post(f"{URL}/use_prc_template",
                        headers=headers,
...
def test_bulk_prc_template(access_token, format="pdf"):

    prc_forms = [_get_sample_prc_form(i, format) for i in range(0, 10)]
    headers = {"content-type": "application/json",
               "Authorization": ("Bearer " + access_token)}
    res = requests.post(f"{URL}/use_prc_template_in_bulk",
                        headers=headers,
...
def test_bulk_basic_template(access_token, format="pdf"):
    ...
    headers = {"content-type": "application/json",
               "Authorization": ("Bearer " + access_token)}
    res = requests.post(f"{URL}/use_basic_template_in_bulk",
                        headers=headers,
...
def test_pas_template(access_token, format="pdf"):
    pas_form = _get_sample_pas_form(0, format)
    headers = {"content-type": "application/json",
               "Authorization": ("Bearer " + access_token)}
    res = requests.post(f"{URL}/use_pas_template",
                        headers=headers,
...
def test_pam_template(access_token, format="pdf"):
    pam_form = _get_sample_pam_form(0, format)
    headers = {"content-type": "application/json",
               "Authorization": ("Bearer " + access_token)}
    res = requests.post(f"{URL}/use_pam_template",
                        headers=headers,
...
def test_bulk_pas_template(access_token, format="pdf"):
    pas_forms = [_get_sample_pas_form(i, format) for i in range(0, 3)]
    headers = {"content-type": "application/json",
               "Authorization": ("Bearer " + access_token)}
    res = requests.post(f"{URL}/use_pas_template_in_bulk",
                        headers=headers,
...
def test_bulk_pam_template(access_token, format="pdf"):
    pam_forms = [_get_sample_pam_form(i, format) for i in range(0, 3)]
    headers = {"content-type": "application/json",
               "Authorization": ("Bearer " + access_token)}
    res = requests.post(f"{URL}/use_pam_template_in_bulk",
                        headers=headers,
...
def test_bulk_speed(access_token, bulk_cnt, format="pdf"):

    start = time.time()
    prc_forms = [_get_sample_prc_form(i, format)
                 for i in range(0, bulk_cnt)]
    headers = {"content-type": "application/json",
               "Authorization": ("Bearer " + access_token)}
    res = requests.post(f"{URL}/use_prc_template_in_bulk",
                        headers=headers,
...
if __name__ == "__main__":
    ...
    test_basic_template(access_token, format="png")

    test_prc_template(access_token, "pdf")
    test_prc_template(access_token, "svg")
    test_prc_template(access_token, "png")

    test_bulk_prc_template(access_token)

    test_bulk_basic_template(access_token)

    test_pas_template(access_token)
    test_pas_template(access_token, "png")
    test_bulk_pas_template(access_token)

    test_pam_template(access_token)
    test_pam_template(access_token, "png")
    test_bulk_pam_template(access_token)

    data = [["cnt", "time_tot", "time_avg"]]
    for i in range(5):  # 5 batches of experiments
        for bulk_cnt in [1, 5, 10, 20, 30, 40, 50]:
            d = test_bulk_speed(access_token, bulk_cnt)
